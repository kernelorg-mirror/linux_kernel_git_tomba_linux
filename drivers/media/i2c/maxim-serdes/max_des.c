...
static int max_des_link_should_keep_vc(struct max_des_priv *priv,
				       struct v4l2_subdev *sd,
			               unsigned int modes)
{
	struct max_des *des = priv->des;
					  ...
					  const struct v4l2_subdev_krouting *routing)
{
	...
	for_each_active_route(routing, route) {
		...
		keep_vc = max_des_link_should_keep_vc(priv, source->sd, modes);
	...
	*num_vc_remaps = 0;
	...
	for (phy_id = 0; phy_id < des->ops->num_phys; phy_id++) {
		...
		for_each_set_bit(src_vc_id, &mask, MAX_SERDES_VC_ID_NUM) {
			...
			ret = max_des_get_src_dst_vc_id(context, pipe->index, phy_id,
							src_vc_id, &dst_vc_id);
			if (ret)
				return ret;
				  ...
				  u32 pad, u64 updated_streams_mask, bool enable)
{
	...
	ret = max_des_set_vc_remaps(priv, &context);
	if (ret)
		return ret;
