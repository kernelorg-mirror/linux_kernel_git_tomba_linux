...
static u32 rkisp1_tpg_calc_frame_sync(const struct v4l2_fract *interval,
				      u32 width, u32 height)
{
	...
	tmp = mult_frac(RKISP1_TPG_CLOCK_RATE, interval->numerator,
...
static void rkisp1_tpg_config_regs(struct rkisp1_tpg *tpg)
{
	...
	state = v4l2_subdev_get_locked_active_state(&tpg->sd);
	...
	if (fmt->width == 1920 && fmt->height == 1080 &&
		...
		sync = rkisp1_tpg_calc_frame_sync(interval, fmt->width,
						  fmt->height);

		val = RKISP1_TPG_W_H(fmt->width + sync, fmt->height + sync);
...
static int rkisp1_tpg_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_TEST_PATTERN:
		...
		break;
	default:
		dev_info(rkisp1->dev, "ctrl(id:0x%x,val:0x%x) is not handled\n",
			 ctrl->id, ctrl->val);
		ret = -EINVAL;
		break;
	}
...
static int rkisp1_tpg_init_state(struct v4l2_subdev *sd,
				 struct v4l2_subdev_state *state)
{
	struct v4l2_mbus_framefmt *fmt;
	struct v4l2_fract *interval;
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	src_fmt = v4l2_subdev_state_get_format(state, 0);
	...
	src_fmt->width = clamp_t(u32, fmt->format.width,
				 RKISP1_TPG_MIN_WIDTH, RKISP1_TPG_MAX_WIDTH);
	src_fmt->height = clamp_t(u32, fmt->format.height,
				  RKISP1_TPG_MIN_HEIGHT, RKISP1_TPG_MAX_HEIGHT);
...
static int rkisp1_tpg_set_frame_interval(struct v4l2_subdev *sd,
					 struct v4l2_subdev_state *state,
					 struct v4l2_subdev_frame_interval *fi)
{
	...
	sync = rkisp1_tpg_calc_frame_sync(&fi->interval, fmt->width,
					  fmt->height);
	 ...
	 */
	fi->interval.numerator = (fmt->width + sync) * (fmt->height + sync);
	fi->interval.denominator = RKISP1_TPG_CLOCK_RATE;
...
static const struct v4l2_subdev_pad_ops rkisp1_tpg_pad_ops = {
	...
	.set_frame_interval = rkisp1_tpg_set_frame_interval,
};

static const struct v4l2_subdev_core_ops rkisp1_core_ops = {
	.subscribe_event = v4l2_ctrl_subdev_subscribe_event,
	.unsubscribe_event = v4l2_event_subdev_unsubscribe,
};

static const struct v4l2_subdev_internal_ops rkisp1_tpg_internal_ops = {
	.init_state = rkisp1_tpg_init_state,
};

static const struct v4l2_subdev_ops rkisp1_tpg_ops = {
	.video = &rkisp1_tpg_video_ops,
	.core = &rkisp1_core_ops,
...
int rkisp1_tpg_register(struct rkisp1_device *rkisp1)
{
	...
	sd->internal_ops = &rkisp1_tpg_internal_ops;
