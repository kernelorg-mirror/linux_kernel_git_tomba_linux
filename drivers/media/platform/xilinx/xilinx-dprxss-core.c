...
static void config_rx_dec_clk(struct xdprxss_state *dp, int bw_code)
{
	 ...
	 */
	hightime = o_val / XDPRX_MMCM_O_VAL_HIGHTIME_DIVIDER;
	reg = XDPRX_MMCM_REG3_PREDIV2 | XDPRX_MMCM_REG3_USED | XDPRX_MMCM_REG3_MX;
	if (o_val % XDPRX_MMCM_O_VAL_EDGE_DIVIDER > 1)
		reg |= BIT(8);

	p5_fedge_en = o_val % XDPRX_MMCM_O_VAL_FEDGE_DIVIDER;
	...
	reg = hightime | hightime << XDPRX_MMCM_MDO_VAL_HIGHTIME_SHIFT;
	xdprxss_mmcm_write(dp, XDPRX_MMCM_REG4_OFFSET, reg);

	/* Implement D */
	reg = 0;
	div_edge = d_val % XDPRX_MMCM_D_VAL_EDGE_DIVIDER;
	hightime = d_val / XDPRX_MMCM_D_VAL_HIGHTIME_DIVIDER;
	...
	div_edge = m_val % XDPRX_MMCM_M_VAL_EDGE_DIVIDER;
	hightime = m_val / XDPRX_MMCM_M_VAL_HIGHTIME_DIVIDER;
	reg = hightime | hightime << XDPRX_MMCM_MDO_VAL_HIGHTIME_SHIFT;
...
static int xdprxss_get_stream_properties(struct xdprxss_state *state, u8 stream)
{
	...
	framerate = (recv_clk_freq * HZ_PER_MHZ) / (hres_total * vres_total);
...
static void xdprxss_core_init(struct xdprxss_state *xdprxss)
{
	...
	xdprxss_write(xdprxss, XDPRX_AUX_CLKDIV_REG, axi_clk / HZ_PER_MHZ);
			      ...
			      struct v4l2_subdev_state *sd_state)
{
	...
	/*
	 * The format is dictated by the source, the default is only what the
	 * pads report until a stream has been detected. Use the first media
	 * bus code supported for the bits per component the sink advertises,
	 * the same way .enum_mbus_code() picks its list.
	 */
...
{
	struct xdprxss_state *state = to_xdprxssstate(subdev);
...
static int dprx_hdcp2x_dpcd_rd_handler(void *ref, u32 offset, u8 *buff, u32 buff_size)
{
	...
	do {
		alignment = reg_offset & XDPRX_ALIGN_FOR_RDWR;
		num_this_time = XDPRX_BYTES_PER_RDWR;

		if (alignment)
			num_this_time = XDPRX_BYTES_PER_RDWR - alignment;
		if (num_this_time > buff_size)
			num_this_time = buff_size;

		value = xdprxss_read(xdprxss, (reg_offset & ~XDPRX_ALIGN_FOR_RDWR));
		if (alignment)
				       ...
				       u32 buff_size)
{
	...
	do {
		...
		if (num_this_time == XDPRX_BYTES_PER_RDWR) {
			for (idx = XDPRX_ALIGN_FOR_RDWR; idx >= 0; idx--) {
				value <<= BITS_PER_BYTE;
				value |= write_buf[idx];
			}
		} else {
				       ...
				       u32 buff_size)
{
	...
	do {
		...
		xdprxss_write(xdprxss, (reg_offset & ~XDPRX_ALIGN_FOR_RDWR), value);
...
static int xdprxss_probe(struct platform_device *pdev)
{
	...
	/*
	 * Allocate the active state before the interrupt handler is registered
	 * and the subdev is exposed to userspace, as it stores the format of
	 * every pad.
	 */
