...
#define OV1063X_PAD_SOURCE 0
#define OV1063X_PAD_VIDEO 1
#define OV1063X_PAD_EMBEDDED 2
...
static void ov1063x_init_formats(struct v4l2_subdev_state *state)
{
	...
	format = v4l2_subdev_state_get_format(state, OV1063X_PAD_VIDEO, 0);
	...
	*v4l2_subdev_state_get_format(state, OV1063X_PAD_SOURCE, 0) = *format;

	if (state->routing.routes[1].flags & V4L2_SUBDEV_ROUTE_FL_ACTIVE) {
		format = v4l2_subdev_state_get_format(state, OV1063X_PAD_EMBEDDED, 0);
		...
		*v4l2_subdev_state_get_format(state, OV1063X_PAD_SOURCE, 1) = *format;
	}
}
				...
				bool enable_embedded_data)
{
	struct v4l2_subdev_route routes[] = {
		{
			.sink_pad = OV1063X_PAD_VIDEO,
			.sink_stream = 0,
			.source_pad = OV1063X_PAD_SOURCE,
			.source_stream = 0,
			.flags = V4L2_SUBDEV_ROUTE_FL_ACTIVE,
		},
		{
			.sink_pad = OV1063X_PAD_EMBEDDED,
			.sink_stream = 0,
			.source_pad = OV1063X_PAD_SOURCE,
			.source_stream = 1,
			.flags = 0,
...
static int ov1063x_init_state(struct v4l2_subdev *sd,
			      struct v4l2_subdev_state *state)
{
	struct ov1063x_priv *priv = to_ov1063x(sd);
	int ret;
...
{
	struct ov1063x_priv *priv = to_ov1063x(sd);
	struct v4l2_mbus_framefmt pix_format;
	struct v4l2_mbus_framefmt emb_format;
	struct v4l2_mbus_framefmt *format;
	...
	/* Format can only be set on internal video pad */
	if (fmt->pad != OV1063X_PAD_VIDEO)
	...
	pix_format.width = fsize->width;
	pix_format.height = fsize->height;
	...
	*format = pix_format;
	...
	if (state->routing.routes[1].flags & V4L2_SUBDEV_ROUTE_FL_ACTIVE) {
		...
		emb_format = *format;
		emb_format.width = pix_format.width;
...
static const struct v4l2_subdev_internal_ops ov1063x_internal_ops = {
	.init_state = ov1063x_init_state,
};
...
static int ov1063x_probe(struct i2c_client *client)
{
	...
	sd->internal_ops = &ov1063x_internal_ops;
	...
	priv->pads[OV1063X_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	priv->pads[OV1063X_PAD_VIDEO].flags = MEDIA_PAD_FL_INTERNAL | MEDIA_PAD_FL_SINK;
	priv->pads[OV1063X_PAD_EMBEDDED].flags = MEDIA_PAD_FL_INTERNAL | MEDIA_PAD_FL_SINK;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;
	ret = media_entity_pads_init(&sd->entity, 3, priv->pads);
	if (ret < 0)
		goto err_ctrls;
