...
#define UB960_MAX_NPORTS	6

#define UB960_NUM_GPIOS		7  /* Physical GPIO pins */
#define UB960_NUM_BC_GPIOS	4  /* Max GPIOs on Back Channel */

#define UB960_NUM_SLAVE_ALIASES	8
#define UB960_MAX_POOL_ALIASES	(UB960_MAX_RX_NPORTS * UB960_NUM_SLAVE_ALIASES)

/* TODO: support all TX ports */
#define UB960_CSI_PORT		0
...
struct ub960_data {
	...
	struct i2c_atr		*atr;
	struct ub960_rxport	*rxport[UB960_MAX_RX_NPORTS];
	struct ub960_csitxport	csitxport;
...
static int ub960_update_bits_shared(const struct ub960_data *priv, u8 reg, u8 mask, u8 val)
{
	struct device *dev = &priv->client->dev;
	int ret;
...
static int ub960_rxport_select(struct ub960_data *priv, int nport)
{
	struct device *dev = &priv->client->dev;
	int ret;
	...
	ret = regmap_write(priv->regmap, UB960_SR_FPD3_PORT_SEL, (nport << 4) | (1 << nport));
	if (ret) {
...
static int ub960_rxport_read(struct ub960_data *priv, int nport, u8 reg, u8 *val)
{
	struct device *dev = &priv->client->dev;
	unsigned int v;
	int ret;
...
static int ub960_rxport_write(struct ub960_data *priv, int nport, u8 reg, u8 val)
{
	struct device *dev = &priv->client->dev;
	int ret;
...
static int ub960_rxport_update_bits(struct ub960_data *priv, int nport,
...
static int ub960_csiport_select(struct ub960_data *priv, int nport)
{
	struct device *dev = &priv->client->dev;
	int ret;
	...
	ret = regmap_write(priv->regmap, UB960_SR_CSI_PORT_SEL, (nport << 4) | (1 << nport));
	if (ret) {
...
static int ub960_csiport_read(struct ub960_data *priv, int nport, u8 reg, u8 *val)
{
	struct device *dev = &priv->client->dev;
	unsigned int v;
	int ret;
...
static int ub960_csiport_write(struct ub960_data *priv, int nport, u8 reg, u8 val)
{
	struct device *dev = &priv->client->dev;
	int ret;
...
#if 0
static int ub960_csiport_update_bits(struct ub960_data *priv, int nport,
				     u8 reg, u8 mask, u8 val)
{
	struct device *dev = &priv->client->dev;
	int ret;
...
static int ub960_write_ind8(const struct ub960_data *priv, u8 reg, u8 val)
{
	...
	err = ub960_write(priv, UB960_SR_IND_ACC_ADDR, reg);
	if (!err)
		err = ub960_write(priv, UB960_SR_IND_ACC_DATA, val);
	return err;
}
...
static int ub960_write_ind16(const struct ub960_data *priv, u8 reg, u16 val)
{
	...
	err = ub960_write(priv, UB960_SR_IND_ACC_ADDR, reg);
	if (!err)
		err = ub960_write(priv, UB960_SR_IND_ACC_DATA, val >> 8);
	if (!err)
		err = ub960_write(priv, UB960_SR_IND_ACC_DATA, val & 0xff);
	return err;
}
				  ...
				  u16 *alias_id)
{
	...
	struct ub960_rxport *rxport = priv->rxport[chan_id];
	struct device *dev = &priv->client->dev;
	u16 alias = 0;
	int reg_idx;
	int pool_idx;
	...
	for (reg_idx = 0; reg_idx < UB960_NUM_SLAVE_ALIASES; reg_idx++) {
		...
		err = ub960_rxport_read(priv, chan_id,
	...
	ub960_rxport_write(priv, chan_id,
			   UB960_RR_SLAVE_ID(reg_idx), client->addr << 1);
	ub960_rxport_write(priv, chan_id,
			   UB960_RR_SLAVE_ALIAS(reg_idx), alias << 1);
...
{
	struct device *dev = &priv->client->dev;
	struct ub960_csitxport *csitxport = &priv->csitxport;
	int ret;

	/* TODO: support all TX ports */
	if (nport > priv->hw_data->num_rxports) {
		dev_err(dev, "Only one TX port supported\n");
		return -EOPNOTSUPP;
	}
...
static void ub960_csi_handle_events(struct ub960_data *priv, int nport)
{
	struct device *dev = &priv->client->dev;
	u8 csi_tx_isr;
	int err;

	err = ub960_csiport_read(priv, nport, UB960_TR_CSI_TX_ISR, &csi_tx_isr);
...
static int ub960_gpio_direction_out(struct gpio_chip *chip,
				    unsigned int offset, int value)
{
	struct ub960_rxport *rxport = gpiochip_get_data(chip);
	unsigned int reg_addr = UB960_RR_BC_GPIO_CTL(offset / 2);
	unsigned int reg_shift = (offset % 2) * 4;

	ub960_rxport_update_bits(rxport->priv, rxport->nport, reg_addr,
				 0xf << reg_shift,
				(0x8 + !!value) << reg_shift);
	return 0;
}

static void ub960_gpio_set(struct gpio_chip *chip,
			   unsigned int offset, int value)
{
	ub960_gpio_direction_out(chip, offset, value);
}

static int ub960_gpio_of_xlate(struct gpio_chip *chip,
...
static int ub960_gpiochip_probe(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
	...
	gc->base                = -1;
	gc->can_sleep           = 1;
	gc->ngpio               = UB960_NUM_BC_GPIOS;
	gc->direction_output    = ub960_gpio_direction_out;
	gc->set                 = ub960_gpio_set;
	gc->of_xlate            = ub960_gpio_of_xlate;
...
static void ub960_gpiochip_remove(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
	struct gpio_chip *gc = &rxport->gpio_chip;
...
static struct attribute *ub960_rxport0_attrs[] = {
	&dev_attr_locked[0].attr,
	&dev_attr_status[0].attr,
	NULL
};

static struct attribute *ub960_rxport1_attrs[] = {
	&dev_attr_locked[1].attr,
	&dev_attr_status[1].attr,
	NULL
};

static struct attribute *ub960_rxport2_attrs[] = {
	&dev_attr_locked[2].attr,
	&dev_attr_status[2].attr,
	NULL
};

static struct attribute *ub960_rxport3_attrs[] = {
			   ...
			   char *buf)
{
	...
	const struct ub960_rxport *rxport = priv->rxport[nport];
...
static struct attribute_group ub960_rxport_attr_group[] = {
	{ .name = "rx0", .attrs = ub960_rxport0_attrs },
	{ .name = "rx1", .attrs = ub960_rxport1_attrs },
	{ .name = "rx2", .attrs = ub960_rxport2_attrs },
	{ .name = "rx3", .attrs = ub960_rxport3_attrs },
};
 ...
 * up in calling ub960_atr_attach_client() where the lock is taken.
 */
static int ub960_rxport_add_serializer(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
	struct device *dev = &priv->client->dev;
...
static void ub960_rxport_remove_serializer(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
 ...
 */
static int
ub960_of_get_reg(struct device_node *np, const char *serializer_name)
{
	u32 alias;
	int ret;
	int idx;
...
static int ub960_rxport_probe_one(struct ub960_data *priv,
				 ...
				 unsigned int nport)
{
	const char *rxport_names[UB960_MAX_RX_NPORTS] = { "rxport0", "rxport1",
							  "rxport2",
							  "rxport3" };
	...
	ret = ub960_of_get_reg(priv->client->dev.of_node, rxport_names[nport]);
	if (ret < 0)
		goto err_node_put;
	...
	ub960_write(priv, UB960_SR_I2C_RX_ID(nport),
	 ...
	 */
	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG, 0x7, bc_freq_val);
	...
	case RXPORT_MODE_RAW10:
		...
		ub960_rxport_update_bits(priv, nport, UB960_RR_PORT_CONFIG, 0x3, 0x3);

		// RAW10_8BIT_CTL = 0b11 : 8-bit processing using lower 8 bits
		// 0b10 : 8-bit processing using upper 8 bits
		ub960_rxport_update_bits(priv, nport, UB960_RR_PORT_CONFIG2, 0x3 << 6, 0x2 << 6);

		break;

	case RXPORT_MODE_CSI2:
		/* CSI-2 Mode (DS90UB953-Q1 compatible) */
		ub960_rxport_update_bits(priv, nport, UB960_RR_PORT_CONFIG, 0x3, 0x0);
	...
	ret = ub960_gpiochip_probe(priv, nport);
	if (ret)
	...
	ub960_rxport_write(priv, nport,
			   UB960_RR_SER_ALIAS_ID, rxport->ser_alias << 1);

	dev_dbg(dev, "ser%d: at alias 0x%02x\n", nport, rxport->ser_alias);

	ret = sysfs_create_group(&dev->kobj, &ub960_rxport_attr_group[nport]);
	if (ret) {
...
err_remove_sysfs:
	sysfs_remove_group(&dev->kobj, &ub960_rxport_attr_group[nport]);
err_remove_gpiochip:
	ub960_gpiochip_remove(priv, nport);
err_unreg_i2c_dev:
...
static void ub960_rxport_remove_one(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
	struct device *dev = &priv->client->dev;
...
static void ub960_rxport_handle_events(struct ub960_data *priv, int nport)
{
	struct ub960_rxport *rxport = priv->rxport[nport];
	struct device *dev = &priv->client->dev;
	u8 rx_port_sts1;
	...
	if (!err)
		err = ub960_rxport_read(priv, nport, UB960_RR_RX_PORT_STS1, &rx_port_sts1);
	if (!err)
		err = ub960_rxport_read(priv, nport, UB960_RR_RX_PORT_STS2, &rx_port_sts2);
	if (!err)
		err = ub960_rxport_read(priv, nport, UB960_RR_CSI_RX_STS, &csi_rx_sts);
	if (!err)
		err = ub960_rxport_read(priv, nport, UB960_RR_BCC_STATUS, &bcc_sts);
	...
	if (rx_port_sts2 & UB960_RR_RX_PORT_STS2_LINE_LEN_CHG) {
		...
		err = ub960_rxport_read(priv, nport, UB960_RR_LINE_LEN_1, &h);
		err = ub960_rxport_read(priv, nport, UB960_RR_LINE_LEN_0, &l);
	...
	if (rx_port_sts2 & UB960_RR_RX_PORT_STS2_LINE_CNT_CHG) {
		...
		err = ub960_rxport_read(priv, nport, UB960_RR_LINE_COUNT_HI, &h);
		err = ub960_rxport_read(priv, nport, UB960_RR_LINE_COUNT_LO, &l);
	...
	locked = rx_port_sts1 & UB960_RR_RX_PORT_STS1_LOCK_STS;
	if (locked && !rxport->locked) {
		dev_dbg(dev, "rx%d: LOCKED\n", nport);
		/* See note about locking in ub960_rxport_add_serializer()! */
		ub960_rxport_add_serializer(priv, nport);
		sysfs_notify(&dev->kobj,
			     ub960_rxport_attr_group[nport].name, "locked");
...
static int ub960_start_streaming(struct ub960_data *priv)
{
	...
	for (i = 0; i < routing->num_routes; ++i) {
		...
		struct ub960_rxport *rxport = priv->rxport[port];
		...
		/* we are only interested in parallel busses */
		...
		fmt = v4l2_state_get_stream_format(state, port, route->sink_stream);
		if (!fmt) {
			ret = -EPIPE;
			break;
		}
	...
	/* TODO: support all TX ports */
	ub960_csiport_write(priv, UB960_CSI_PORT, UB960_TR_CSI_CTL, csi_ctl);
	...
	for (nport = 0; nport < priv->hw_data->num_rxports; ++nport) {
		...
		/* TODO: support forwarding to any TX port */
	}
...
static int ub960_stop_streaming(struct ub960_data *priv)
{
	...
	/* Disable forwarding */
	ub960_write(priv, UB960_SR_FWD_CTL1, (BIT(0) | BIT(1) | BIT(2) | BIT(3)) << 4);

	/* Stop all cameras */
	for (i = 0; i < priv->hw_data->num_rxports; ++i) {
		struct ub960_rxport *rxport = priv->rxport[i];
	...
	/* TODO: support all TX ports */
	ub960_csiport_write(priv, UB960_CSI_PORT, UB960_TR_CSI_CTL, 0);
				...
				struct v4l2_mbus_frame_desc *fd)
{
	...
	dev_dbg(&priv->client->dev, "%s for pad %d\n", __func__, pad);

	if (pad != priv->hw_data->num_rxports)	/* first tx pad */
		return -EINVAL;
	...
	for (i = 0; i < routing->num_routes; ++i) {
		...
		fd->entry[fd->num_entries].length =
			fmt->width * fmt->height * ub960_fmt->bpp / 8;
		...
		fd->entry[fd->num_entries].bus.csi2.dt = ub960_fmt->datatype;
		fd->num_entries++;

		dev_dbg(&priv->client->dev, "UB FD source %u/%u: %ux%u\n",
			route->sink_pad, route->sink_stream, fmt->width,
			fmt->height);
	}
			 ...
			 struct v4l2_subdev_format *format)
{
	...
	// TODO: implement fmt validation and propagation
			  ...
			  struct v4l2_subdev_state *state)
{
	...
	struct v4l2_subdev_route routes[] = {
		{
			.sink_pad = 0,
			.sink_stream = 0,
			.source_pad = UB960_MAX_RX_NPORTS,
			.source_stream = 0,
	...
	return ub960_set_routing(sd, state, &routing);
}
...
{
	struct ub960_data *priv = container_of(ctrl->handler, struct ub960_data,
					      ctrl_handler);
...
static irqreturn_t ub960_handle_events(int irq, void *arg)
{
	...
	err = ub960_read(priv, UB960_SR_INTERRUPT_STS, &int_sts);
...
static int ub960_parse_dt(struct ub960_data *priv)
{
	...
	for (n = 0; n < priv->hw_data->num_rxports + priv->hw_data->num_txports; ++n) {
		...
		if (n < priv->hw_data->num_rxports)
			err = ub960_rxport_probe_one(priv, ep_np, n);
		else
			err = ub960_csiport_probe_one(priv, ep_np, n);
...
static int ub960_probe(struct i2c_client *client)
{
	...
	priv->hw_data = of_match_device(ub960_dt_ids, dev)->data;
	...
	if (IS_ERR(priv->pd_gpio)) {
		err = PTR_ERR(priv->pd_gpio);
		if (err != -EPROBE_DEFER)
			dev_err(dev, "Cannot get powerdown GPIO (%d)", err);
		return err;
	}
	...
	err = ub960_read(priv, UB960_SR_REV_MASK, &rev_mask);
	if (err) {
		dev_err(dev, "Cannot read first register (%d), abort\n", err);
		goto err_reg_read;
	}

	err = ub960_atr_probe(priv);
	if (err)
		goto err_atr_probe;

	err = ub960_parse_dt(priv);
	if (err)
		goto err_parse_dt;

	/* V4L2 */

	err = ub960_create_subdev(priv);
	if (err)
		goto err_subdev;
	...
	/* Kick off */
	{
		...
		for (i = 0; i < priv->hw_data->num_rxports; i++) {
			...
			err = ub960_rxport_read(priv, i, UB960_RR_RX_PORT_STS1, &rx_port_sts1);
			if (rx_port_sts1 & UB960_RR_RX_PORT_STS1_LOCK_STS) {
				dev_dbg(dev, "rx%d: INITIAL LOCKED\n", i);
				ub960_rxport_add_serializer(priv, i);
	...
	if (client->irq) {
		...
		ub960_update_bits_shared(priv, UB960_SR_GPIO_INPUT_CTL,
					 BIT(3), 0);
