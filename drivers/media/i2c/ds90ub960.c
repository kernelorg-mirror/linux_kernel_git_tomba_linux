					...
					u8 *val, int *err)
{
	...
	ret = i2c_smbus_xfer(priv->client->adapter, rxport->ser.alias, priv->client->flags,
...
static int ub960_serializer_temp_ramp(struct ub960_rxport *rxport)
{
	...
	/* Enable I2C auto ack on back channel before we set dynamic cfg and reset */
	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG,
