				    ...
				    u32 pad, u64 streams_mask)
{
	...
	if (!xcsi2rxss->enabled_sink_streams) {
		 ...
		 * stopped. Enabling more streams is refused, as recovering
		 * requires disabling all the streams, which resets the core,
		 * and enabling them again.
		 */
		dev_warn(xcsi2rxss->dev,
			 "core disabled by an error, disable all streams to recover\n");
		return -EIO;
	}
