 ...
 * @xhdmi: pointer to driver state
 *
 * Publish the format detected by the hardware through the subdev active
 * state. All callers run in sleepable context and hold no other lock, so the
 * state lock can be taken here. The active state does not exist yet when the
 * hardware is initialised from probe, in which case there is nothing to
 * update.
 */
...
/*
 * The HDMI Rx has to be running before the pipeline is started, as it detects
 * the incoming stream and reports its format, and there is nothing left to
 * configure once the pipeline starts. Enabling and disabling the stream is
 * therefore a no-op.
 */
