 ...
 * times, and no Xilinx video IP can drop data. A copy reaching a consumer that
 * isn't running backpressures the broadcaster and stalls the branches that
 * are. Start the upstream part of the pipeline only once all the source pads
 * with an enabled link have been enabled, and stop it as soon as the first one
 * is disabled. Links can't change while the pipeline streams, so the set of
 * linked pads is stable and can be recomputed on each call.
 */
				...
				u64 streams_mask)
{
	...
	/*
	 * Stop the upstream part of the pipeline, if it is running, before
	 * removing one of its consumers.
	 */
	if (xvbr->enabled_pads == xvbr_linked_pads(xvbr))
