...
#define for_each_active_rxport_fpd4()                                         \
	for (struct ub960_rxport_iter it =                                    \
		     ub960_iter_rxport(priv, (struct ub960_rxport_iter){ 0 }, \
				       UB960_ITER_ACTIVE_ONLY |               \
...
{
	for_each_rxport()
		ub960_rxport_clear_errors(priv, it.nport);
}
...
static void ub960_rxport_lockup_wa_ub9702(struct ub960_data *priv)
{
	/* Toggle PI_MODE to avoid possible FPD RX lockup */
	ub960_update_bits(priv, UB9702_RR_CHANNEL_MODE,
			  GENMASK(4, 3), 2 << 3);
				   ...
				   unsigned int *lock_mask)
{
	...
	while (time_before(jiffies, timeout)) {
		...
		if (fpd4_wa)
			ub960_rxport_lockup_wa_ub9702(priv);
...
static void ub960_init_tx_ports_ub960(struct ub960_data *priv)
{
	u8 speed_select;
...
static void ub960_init_tx_ports_ub9702(struct ub960_data *priv)
{
	u8 speed_select;
	u8 pll_div;
	u8 ana_pll_div;
	...
	ub960_write(priv, UB960_SR_CSI_PLL_CTL, speed_select);
	ub960_write(priv, UB9702_SR_CSI_PLL_DIV, pll_div);
	ub960_write_ind(priv, UB960_IND_TARGET_CSI_ANA,
			UB9702_IR_CSI_ANA_CSIPLL_REG_1, ana_pll_div);
}

static int ub960_init_tx_ports(struct ub960_data *priv)
{
	if (priv->hw_data->is_ub9702)
		ub960_init_tx_ports_ub9702(priv);
	else
		ub960_init_tx_ports_ub960(priv);
...
static void ub960_turn_off_rxport_ub9702(struct ub960_data *priv,
					 unsigned int nport)
{
	/* Disable RX port */
	ub960_update_bits(priv, UB960_SR_RX_PORT_CTL, BIT(nport), 0);

	/* Disable FPD Rx and FPD BC CMR */
	ub960_rxport_write(priv, nport, UB9702_RR_RX_CTL_2, 0x1b);

	/* Disable FPD BC Tx */
	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG, BIT(4), 0);

	/* Disable internal RX blocks */
	ub960_rxport_write(priv, nport, UB9702_RR_RX_CTL_1, 0x15);

	/* Disable AEQ */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_CFG_2, 0x03);

	/* PI disabled and oDAC disabled */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_CFG_4, 0x09);

	/* AEQ configured for disabled link */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_CFG_1, 0x20);

	/* disable AEQ clock and DFE */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_CFG_3, 0x45);

	/* Powerdown FPD3 CDR */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_FPD3_CDR_CTRL_SEL_5, 0x82);
}

static void ub960_set_bc_drv_config_ub9702(struct ub960_data *priv,
					   unsigned int nport)
{
	u8 fpd_bc_ctl0;
	u8 fpd_bc_ctl1;
...
static void ub960_set_fpd4_sync_mode_ub9702(struct ub960_data *priv,
					    unsigned int nport)
{
	/* FPD4 Sync Mode */
	ub960_rxport_write(priv, nport, UB9702_RR_CHANNEL_MODE, 0x0);

	/* BC_FREQ_SELECT=(PLL_FREQ/3200) Mbps */
	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG,
				 UB960_RR_BCC_CONFIG_BC_FREQ_SEL_MASK, 6);

	ub960_set_bc_drv_config_ub9702(priv, nport);

	/* Set AEQ timer to 400us/step */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_SYSTEM_INIT_REG0, 0x2f);
...
static void ub960_set_fpd4_async_mode_ub9702(struct ub960_data *priv,
					     unsigned int nport)
{
	/* FPD4 ASync Mode */
	ub960_rxport_write(priv, nport, UB9702_RR_CHANNEL_MODE, 0x1);

	/* 10Mbps w/ BC enabled */
	/* BC_FREQ_SELECT=(PLL_FREQ/3200) Mbps */
	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG,
				 UB960_RR_BCC_CONFIG_BC_FREQ_SEL_MASK, 2);
...
static void ub960_set_fpd3_sync_mode_ub9702(struct ub960_data *priv,
					    unsigned int nport)
{
	/* FPD3 Sync Mode */
	ub960_rxport_write(priv, nport, UB9702_RR_CHANNEL_MODE, 0x2);
	...
	/* Set AEQ_LOCK_MODE = 1 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_FPD3_AEQ_CTRL_SEL_1, BIT(7));
...
static void ub960_set_raw10_dvp_mode_ub9702(struct ub960_data *priv,
					    unsigned int nport)
{
	/* FPD3 RAW10 Mode */
	ub960_rxport_write(priv, nport, UB9702_RR_CHANNEL_MODE, 0x5);

	ub960_rxport_update_bits(priv, nport, UB960_RR_BCC_CONFIG,
				 UB960_RR_BCC_CONFIG_BC_FREQ_SEL_MASK, 0);
					  ...
					  unsigned int nport)
{
	...
	if (!rxport) {
		ub960_turn_off_rxport_ub9702(priv, nport);
	...
	case RXPORT_CDR_FPD4:
		switch (rxport->rx_mode) {
		case RXPORT_MODE_CSI2_SYNC:
			ub960_set_fpd4_sync_mode_ub9702(priv, nport);
			dev_dbg(dev, "rx%u: FPD-Link IV SYNC mode\n", nport);
			break;
		case RXPORT_MODE_CSI2_NONSYNC:
			ub960_set_fpd4_async_mode_ub9702(priv, nport);
	...
	case RXPORT_CDR_FPD3:
		switch (rxport->rx_mode) {
		case RXPORT_MODE_CSI2_SYNC:
			ub960_set_fpd3_sync_mode_ub9702(priv, nport);
			dev_dbg(dev, "rx%u: FPD-Link III SYNC mode\n", nport);
			break;
		case RXPORT_MODE_RAW10:
			ub960_set_raw10_dvp_mode_ub9702(priv, nport);
				      ...
				      unsigned int nport)
{
	...
	for (unsigned int retry = 0; retry < 3; ++retry) {
		...
		ub960_rxport_read(priv, nport, UB960_RR_RX_PORT_STS1, &port_sts1);
		...
		if (!rx_lock) {
			...
			if (rx_aeq != prev_aeq) {
				...
				dev_dbg(dev, "rx%u: lossy cable detected, RX_AEQ %#x, RX_AEQ_LIMIT %#x, retry %u\n",
					nport, rx_aeq, rx_aeq_limit, retry);
				dev_dbg(dev, "rx%u: will continue with initiation sequence but high AEQ\n", nport);
				return 0;
			}
		} else {
			dev_dbg(dev, "rx%u: locked and AEQ normal before setting AEQ window\n",
				nport);
			return 0;
		}
	}
...
static void ub960_enable_aeq_lms_ub9702(struct ub960_data *priv,
					unsigned int nport)
{
	struct device *dev = &priv->client->dev;
	u8 read_aeq_init;
	...
	/* Set AEQ Min */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_ALP_SEL6, read_aeq_init);
	/* Set AEQ Max */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_ALP_SEL7, read_aeq_init + 1);
	/* Set AEQ offset to 0 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_AEQ_ALP_SEL10, 0x0);

	/* enable AEQ tap2 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_EQ_CTRL_SEL_38, 0x00);
	/* Set VGA Gain 1 Gain 2 override to 0 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_VGA_CTRL_SEL_8, 0x00);
	/* Set VGA Initial Sweep Gain to 0 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_VGA_CTRL_SEL_6, 0x80);
	/* Set VGA_Adapt (VGA Gain) override to 0 (thermometer encoded) */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_VGA_CTRL_SEL_3, 0x00);
	/* Enable VGA_SWEEP */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_EQ_ADAPT_CTRL, 0x40);
	/* Disable VGA_SWEEP_GAIN_OV, disable VGA_TUNE_OV */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_EQ_OVERRIDE_CTRL, 0x00);

	/* Set VGA HIGH Threshold to 43 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_VGA_CTRL_SEL_1, 0x2b);
	/* Set VGA LOW Threshold to 18 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_VGA_CTRL_SEL_2, 0x12);
	/* set vga_sweep_th to 32 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_EQ_CTRL_SEL_15, 0x20);
	/* set AEQ timer to 400us/step and parity threshold to 7 */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_SYSTEM_INIT_REG0, 0xef);

	dev_dbg(dev, "rx%u: enable FPD-Link IV AEQ LMS\n", nport);
}

static void ub960_enable_dfe_lms_ub9702(struct ub960_data *priv,
					unsigned int nport)
{
	struct device *dev = &priv->client->dev;
	...
	/* enable DFE LMS */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_EQ_CTRL_SEL_24, 0x40);
	/* Disable VGA Gain1 override */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_GAIN_CTRL_0, 0x20);

	usleep_range(1000, 5000);

	/* Disable VGA Gain2 override */
	ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(nport),
			UB9702_IR_RX_ANA_GAIN_CTRL_0, 0x00);
...
static int ub960_init_rx_ports_ub9702(struct ub960_data *priv)
{
	...
	if (have_fpd4) {
		for_each_active_rxport_fpd4() {
			...
			ub960_rxport_write(priv, it.nport, UB9702_RR_RX_SM_SEL_2, 0x0);
		...
		for_each_active_rxport_fpd4() {
			ub960_write_ind(priv, UB960_IND_TARGET_RX_ANA(it.nport),
					UB9702_IR_RX_ANA_AEQ_ALP_SEL7, 0x23);
			dev_dbg(dev, "rx%u: AEQ restart\n", it.nport);
		}

		dev_dbg(dev, "sleep 20ms for FPD-Link IV Lock\n");
		msleep(20);
		...
		for_each_active_rxport_fpd4()
			ub960_enable_aeq_lms_ub9702(priv, it.nport);
		...
		for_each_active_rxport_fpd4() {
			/* Release State Machine */
			ub960_rxport_write(priv, it.nport, UB9702_RR_RX_SM_SEL_2, 0);
		}
	}
	...
	// XXX TODO: ub95x_temp_ramp_ext()

	for_each_active_rxport_fpd4()
		ub960_enable_dfe_lms_ub9702(priv, it.nport);
	...
	ub960_write(priv, UB9702_SR_CSI_EXCLUSIVE_FWD2, 0x18);
	// XXX the above sets bits 3:0 to 0
	//ub960_update_bits(priv, UB9702_SR_CSI_EXCLUSIVE_FWD2, GENMASK(5, 4), BIT(4));

	for_each_active_rxport_fpd4() {
		u8 final_aeq;

		ub960_read_ind(priv, UB960_IND_TARGET_RX_ANA(it.nport),
					       ...
					       struct v4l2_subdev_state *state)
{
	...
	for_each_active_rxport() {
		...
		case RXPORT_MODE_RAW10:
			...
			ub960_rxport_write(priv, it.rxport->nport,
...
static void ub960_log_status_ub960_sp_eq(struct ub960_data *priv, unsigned int nport)
{
	struct device *dev = &priv->client->dev;
	u8 eq_level;
	s8 strobe_pos;
...
static irqreturn_t ub960_handle_events(int irq, void *arg)
{
	...
	for_each_active_rxport() {
		if (int_sts & UB960_SR_INTERRUPT_STS_IS_RX(it.nport))
			ub960_rxport_handle_events(priv, it.nport);
	}
...
static int ub960_probe(struct i2c_client *client)
{
	...
	for (unsigned int i = 0; i < 4; ++i)
		ub960_write(priv, UB960_SR_I2C_RX_ID(i), (UB960_DEBUG_I2C_RX_ID + i) << 1);
#endif
