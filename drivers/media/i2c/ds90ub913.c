...
static void ub913_reset(struct ub913_data *priv, bool reset_regs)
{
	struct device *dev = &priv->client->dev;
	...
	ub913_write(priv, UB913_REG_RESET_CTL, bit);
	...
	while (time_before(jiffies, timeout)) {
		...
		ret = regmap_read(priv->regmap, UB913_REG_RESET_CTL, &v);
		if (!ret && !(v & bit))
			return;
...
static int ub913_probe(struct i2c_client *client)
{
	...
	dev_dbg(dev, "mode from %s: %#x\n", mode_override ? "reg" : "deserializer",
		mode);

	ub913_reset(priv, true);
