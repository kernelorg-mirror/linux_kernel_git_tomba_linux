...
The kernel tree:
    ...
    git://git.kernel.org/pub/scm/linux/kernel/git/tomba/linux.git multistream-milestone-3.1

DT Overlay tree:

    git://git.kernel.org/pub/scm/linux/kernel/git/tomba/linux-dt.git multistream-milestone-3.1

kms++ tree:

    git://github.com/tomba/kmsxx.git multistream-milestone-3.1
...
When a pad can contain multiple streams, the above doesn't anymore make sense as there's no single configuration for a pad. The multiplexed streams support changes the configuration to be based on pad+stream tuples. The userspace needs to first configure the routes, which defines the streams. The userspace must then configure each pad+stream tuple using the subdev ioctls.
...
- Set V4L2_SUBDEV_FL_MULTIPLEXED flag for the subdev
- Change the v4l2_subdev_pad_ops to handle the `stream` field.
...
Sensors can provide embedded data (metadata) either above or below the actual pixel content for each frame.  Even if a sensor driver doesn't have routes as such, the driver must implement routing and use `V4L2_SUBDEV_ROUTE_FL_SOURCE` flag for its routes. This is needed to expose the streams on the sensor's source pads.
...
- The userspace must configure a "stream", which starts from a pad in a sensor, and goes to a video device by configuring the pads and the routing on all subdevs along the pipeline.
...
No changes wrt. routing has yet been made to the standard v4l tools. To use multistream support you have to write an application using the new features (routing and streams).

For my work I have used tools I'm most familiar with: kms++ library with python bindings. I have added C++ classes for the required v4l features, and added python bindings for them. I have a small python application (py/tests/cam-mplex.py) that sets up the pipeline according to the given configuration, and shows the data on screen using KMS planes.
...
CAL driver is an old driver and adding Media Controller (MC) support with the non-MC support has proven to be very difficult. The solution was to add a new module parameter, `cal_mc_api`, which enables the MC support. For multiplexed streams, MC API is a requirement.
...
This is needed to add TRY support for routing. The subdev driver's need both the pad configuration and the routing table to be able to support TRY properly.
