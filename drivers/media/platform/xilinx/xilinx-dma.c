 ...
 *
 * Start or stop streaming on the subdev connected to the DMA video node. The
 * subdev propagates the stream state further up the pipeline.
 *
 * Return: 0 if successful, -EPIPE if no subdev is connected to the DMA, or the
 * return value of the failed v4l2_subdev_enable_streams() operation. Stopping
 * is best effort and always succeeds.
 */
static int xvip_dma_set_remote_stream(struct xvip_dma *dma, bool start)
{
	...
	if (ret < 0) {
		...
		/*
		 * Stopping is best effort, keep remote_streaming in sync with
		 * the hardware.
		 */
		if (start)
			return ret;
 ...
 * @start: Start (when true) or stop (when false) the subdev
 *
 * A subdev that ends the pipeline has no source pad in use, so it can't be
 * started with v4l2_subdev_enable_streams(). Start it with .s_stream()
 * instead, from where its driver propagates the stream state upstream.
 *
 ...
 * @start: Start (when true) or stop (when false) the subdev
 *
 * Counterpart of xvip_dma_set_remote_stream() for the output DMA engines. The
 * branch fed by an output DMA engine may end in a subdev instead of a capture
 * DMA engine, in which case nothing else starts that subdev. Only the subdev
 * connected to the video node is considered, a branch that ends in a subdev
 * further downstream is not covered.
 *
 * Return: 0 if successful, -EPIPE if no subdev is connected to the DMA, or the
 ...
 * capture DMA engines, and the subdevs without a consumer, such as the
 * memory-based scene change detection channels. Each of them propagates the
 * stream state up to the source of its branch. The output DMA engines feed
 * the pipeline and are skipped, the subdevs connected to them are started
 * from the other end of their branch.
 *
