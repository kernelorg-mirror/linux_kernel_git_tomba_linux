...
#define IMX390_PAD_SOURCE 0
#define IMX390_PAD_VIDEO 1
#define IMX390_PAD_EMBEDDED 2
...
static const struct v4l2_mbus_framefmt imx390_default_format = {
	.width = 640,
	.height = 480,
...
static const struct v4l2_mbus_framefmt imx390_default_embedded_format = {
	.width = 640,
...
static void imx390_init_formats(struct v4l2_subdev_state *state)
{
	...
	format = v4l2_subdev_state_get_format(state, IMX390_PAD_VIDEO, 0);
	...
	*format = imx390_default_format;
	...
	if (state->routing.routes[1].flags & V4L2_SUBDEV_ROUTE_FL_ACTIVE) {
		...
		*format = imx390_default_embedded_format;
		...
		format = v4l2_subdev_state_get_format(state, IMX390_PAD_SOURCE, 1);
		*format = imx390_default_embedded_format;
	}
}
			       ...
			       bool enable_embedded_data)
{
	struct v4l2_subdev_route routes[] = {
		{
			.sink_pad = IMX390_PAD_VIDEO,
			.sink_stream = 0,
			.source_pad = IMX390_PAD_SOURCE,
			.source_stream = 0,
			.flags = V4L2_SUBDEV_ROUTE_FL_ACTIVE,
		},
		{
			.sink_pad = IMX390_PAD_EMBEDDED,
			.sink_stream = 0,
			.source_pad = IMX390_PAD_SOURCE,
			.source_stream = 1,
			.flags = 0,
...
static int imx390_probe(struct i2c_client *client)
{
	...
	priv->pads[IMX390_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	priv->pads[IMX390_PAD_VIDEO].flags = MEDIA_PAD_FL_INTERNAL | MEDIA_PAD_FL_SINK;
	priv->pads[IMX390_PAD_EMBEDDED].flags = MEDIA_PAD_FL_INTERNAL | MEDIA_PAD_FL_SINK;
